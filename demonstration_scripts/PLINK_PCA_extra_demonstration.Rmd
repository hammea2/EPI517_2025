...
## Install and load tidyverse, qqman, vcfR, and GGally from CRAN
p_load(tidyverse,qqman,vcfR,GGally)
```
...
plot_data = merge(fam,PCs, by.x = "V2", by.y = "IID")
plot_data$case_status = ifelse(plot_data$V6==1,"control","case") 
```
...
ggparcoord(data = plot_data, columns = 8:13, groupColumn = "case_status")+
  theme_bw()
```
